...
9. [done/09-ci-docs-alignment.md](done/09-ci-docs-alignment.md) ✅ — 完成。Python 句柄关闭 + 本地/CI 回归 + `__pycache__/` 忽略；CI 固定 1.93.1 `msrv` job（`--locked --all-targets` 本地通过）；README/usage/architecture/release 按最终行为同步（含 bundled 快照路径、warning 默认、白名单语义、续轮/预算、SSE 截断、proxy 重试期限与采样限制）。收口时另修复基线 rustdoc 私有 intra-doc 链接失败（02/04/05 引入的 4 处 doc 注释，零逻辑改动，已通过）；provider_proxy 全并行偶发 1 次（cancelled_restart_candidate_is_reaped，重跑全绿，见 plan 执行结果）。依赖：01–08 全部合入。
