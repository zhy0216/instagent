...
| `done/19-agent-module-split.md` ✅ | P2 | hard | 在行为测试护栏下拆分 agent loop 与工具执行职责 |
| `done/20-hooks-provider-split.md` ✅ | P2 | hard | 在契约锁定后拆分 hooks 与 OpenAI transport/parser |
...
19. `done/19-agent-module-split.md` ✅ — 完成。agent/mod.rs 按职责拆分：loop 本体与 provider stream 折叠留在 mod.rs（模块头补职责地图）；事件契约与背压 sink 移入 event.rs；三段式工具执行、并行单元划分、图片预算与预览移入私有模块 exec.rs（4 个纯单测随迁）；`PARALLEL_TOOL_LIMIT` / `SESSION_IMAGE_BUDGET` / `EMIT_GRACE` / `dropped_event_count` 经 re-export 保持原公开路径；公开 API、会话不变量、并行策略与事件契约不变，lib.rs 与既有模块声明未动；11/13 护栏回归测试（取消、并发、残缺 stream、event drop）原样保留，全套 452 通过。依赖：11、13、16。
20. `done/20-hooks-provider-split.md` ✅ — 完成。`hooks.rs` 按四个私有实现单元整理：manifest loading（`load_plugin_hooks`/`register_event_groups`/`hooks_file_path`）、env/policy（`declared_env`/`apply_plugin_env`/`fail_with_warning`，ADR 0003 D2/D3）、command execution（`hook_command`/`run_hook`）、decision parser（`parse_decision`，goose classify_output 行为逐字不变）；`provider/openai.rs` 按 transport 骨架 / request mapping（含图片预算子单元）/ stream state / SSE delta parser（`accumulate_tool_calls`）/ error 与 usage handling（`stream_error_from_chunk`/`usage_from_chunk`/`map_stop_reason`）整理，SSE 驱动继续复用共享层。公开类型、模块树与调用方路径不变，`lib.rs` 与模块声明未动。依赖：06、08、11、13、16。
