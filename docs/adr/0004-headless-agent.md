...
- 有能力装载/预检或生命周期诊断时增加 `diagnostics` 数组，每项含 `code`、
  `source`、`message`；无诊断时省略。它是 schema v1 的可选扩展字段。
...
- stdout/stderr 由独立线程写入，每路进度队列最多保留 1 MiB（含在写内容），
  队列满时丢弃后续进度/诊断，不阻塞任务。文本输出收尾或完整 JSON 交付最多
  等待 1 秒，最后诊断排空最多 250 ms；这部分位于任务执行与资源清理之后。
  JSON 交付必须得到写入及 flush 成功确认，超时同样退出 1。接收方应持续读取两路输出。
...
`extensions.dev.instagent.minKernel` 若声明，必须为 `MAJOR.MINOR` 或
`MAJOR.MINOR.PATCH` 的非负十进制整数版本（不带前导零），省略 patch 视为 0。
不支持版本范围或预发布标签；版本高于当前内核或格式错误时拒绝该插件。
发现阶段沿用带原因跳过的行为，安装时直接报错；被 `--only-plugin` 选中的
不兼容插件会令任务失败，不允许悄悄缺失。

`--only-plugin NAME` 可重复，限定本次任务的已启用插件（须包含 provider 所属插件）；
`--tool NAME` 可重复，限定最终模型可见名称及实际调用，`--no-tools` 选择空工具集。
`--require-tool NAME` 可重复，能力预检发现不存在、加载失败或被排除的工具时，
在创建新会话及请求模型前失败，并返回结构化诊断。预检不替代 Stop hook 的业务验收。
未提供这些选项时保留既有能力集合；选择不修改 settings，也不重新启用被禁用插件。
MCP 初始化跨插件共用最多 4 个并发槽，注册顺序保持确定性；工具清单枚举最多 4 路并发。

完整生命周期由库 `agent::task::run(RunRequest, CancellationToken, Sender<Event>)`
提供，返回 `RunReport`，包含初始化、会话恢复、执行期限、hooks 和清理。
库不安装信号处理器、不直接写终端；日志由宿主配置 tracing。CLI 负责参数、信号和输出。
配置、会话及插件数据目录继续使用既有 INSTAGENT_* 约定；同会话单进程独占约定不变。
...
`run` 不自动拉取或更新插件；部署方显式执行 `plugin update [name]`。
