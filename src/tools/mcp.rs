...
//! 规范规定不承载凭据，远程鉴权 v1 不做，暂不随请求发送）；`sse` 跳过并给
//! "不支持"信息。`initialize` 返回的 `instructions` 存下供系统提示拼装
//! （`16`/`18` 接线）。
//!
...
impl McpSource {
    /// 建立连接并 initialize；`instructions` 从握手结果里取。
        ...
        let (client, child_pid) = match server.r#type {
            ...
            McpServerType::StreamableHttp => {
                ...
                if !server.headers.is_empty() {
                    // 规范规定 headers 不承载凭据；远程鉴权 v1 不做（第三版 §2.3）。
                    tracing::debug!(
                        "MCP server `{id}`: ignoring {} header(s), v1 does not send them",
                        server.headers.len()
                    );
                }
...
{
    Ok(serve_client(LoggingClientHandler, transport).await?)
}
...
pub struct McpLoadOutcome {
    ...
    /// sse 跳过等可读提示（`18` CLI 可展示）。
    pub notes: Vec<String>,
}

/// 一个插件的所有 MCP server：`06` 的 [`load_servers`] 逐个建 [`McpSource`]，
/// sse 跳过并给"不支持"信息。`plugin_data` 是 `${PLUGIN_DATA}` 目录
...
pub async fn connect_plugin(plugin: &Plugin, plugin_data: &Path) -> crate::Result<McpLoadOutcome> {
    ...
    for server in load_servers(plugin, plugin_data)? {
        ...
        outcome
            .sources
            .push(McpSource::connect(plugin, &server).await?);
    }
    Ok(outcome)
}
...
impl ToolSource for McpSource {
    ...
    async fn list(&self) -> Vec<ToolSpec> {
        let Some(client) = self.client_arc() else {
            tracing::warn!("MCP {}: list skipped, server not connected", self.id);
            return Vec::new();
        };
        match client.list_all_tools().await {
            Ok(tools) => tools
                ...
                .collect(),
            Err(e) => {
                tracing::warn!("MCP {}: list_tools failed: {e}", self.id);
                Vec::new()
            }
        }
    }
